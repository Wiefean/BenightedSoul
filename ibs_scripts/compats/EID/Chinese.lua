...
[IBS_ItemID.ChestChest]={
	...
	seijaBuff={
		...
		data = {
			append = function(x) 
				return (x > 1 and "#在新层额外生成"..(x-1).."个金箱子") or ''
			end
		},
	},
},
...
[IBS_ItemID.SilverKey]={
	name='银之钥',
	info='在清理房间后充能，可充满两次'..
		 '#无记录时，记录当前房间类型(仅部分可记录)和游戏计时'..
		 '#已有记录时，尝试将距离最近的门联通的房间重置为记录类型，将游戏计时变为记录的计时，并重置记录'..
		 '#!!! 仅能重置部分未清理的，类型为可记录的或普通房间的房间'..
...
[IBS_ItemID.DanishGambit]={
	name='丹麦弃兵',
	info='无需满充能即可使用'..
		 '#充能未满时使用，将吸收饰品和品质{{Quality1}}1及以下的道具以充能'..
		 '#可为额外充能条充能'..
